...
prior_expr_regex = re.compile(r'(\d+\D+)?(?P<num1>\d+)(?P<action>[/*])(?P<num2>\d+)(\d+\D+)?')
sec_expr_regex = re.compile(r'(\d+\D+)?(?P<num1>\d+)(?P<action>[+-])(?P<num2>\d+)(\d+\D+)?')
actions = {
    '+': operator.add,
    '-': operator.sub,
    '*': operator.mul,
...
def calculate(input_expression: str):
    ...
    for expr in [prior_expr_regex, sec_expr_regex]:
        ...
        if match:
            ...
            return calculate(f'{match.string[:match.start("num1")]}'
                             f'{str(result)}'
